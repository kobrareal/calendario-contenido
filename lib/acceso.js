...
// ---------- acceso por PIN ----------
// El PIN es cómodo pero corto, así que la barrera real no es su largo sino el freno a los
// intentos: sin esto, una página de acceso pública se prueba entera con un script.
const CLAVE_INTENTOS = '__intentos__';
const MAX_INTENTOS = 8;              // por ventana
const VENTANA_MS = 15 * 60 * 1000;   // 15 minutos

async function leerIntentos() {
  const r = await fetch(`${KV()}?key=eq.${encodeURIComponent(CLAVE_INTENTOS)}&select=value`, {
    headers: cabeceras(), cache: 'no-store'
  });
  if (!r.ok) return {};
  const filas = await r.json();
  try { return filas.length ? JSON.parse(filas[0].value) : {}; } catch (e) { return {}; }
}

async function guardarIntentos(datos) {
  await fetch(KV(), {
    method: 'POST',
    headers: cabeceras({ Prefer: 'resolution=merge-duplicates,return=minimal' }),
    body: JSON.stringify({ key: CLAVE_INTENTOS, value: JSON.stringify(datos), updated_at: new Date().toISOString() })
  });
}

// Devuelve {bloqueado, restantes}. Cuenta por IP y limpia lo vencido al pasar.
async function registrarIntento(ip, exitoso) {
  const ahora = Date.now();
  const datos = await leerIntentos();
  Object.keys(datos).forEach(k => { if (!datos[k] || datos[k].hasta < ahora) delete datos[k]; });

  if (exitoso) {
    delete datos[ip];
    await guardarIntentos(datos);
    return { bloqueado: false, restantes: MAX_INTENTOS };
  }

  const previo = datos[ip] && datos[ip].hasta > ahora ? datos[ip] : { n: 0, hasta: ahora + VENTANA_MS };
  previo.n += 1;
  datos[ip] = previo;
  await guardarIntentos(datos);
  return { bloqueado: previo.n >= MAX_INTENTOS, restantes: Math.max(0, MAX_INTENTOS - previo.n) };
}

async function estaBloqueado(ip) {
  const datos = await leerIntentos();
  const e = datos[ip];
  return !!(e && e.hasta > Date.now() && e.n >= MAX_INTENTOS);
}

// Busca a quién corresponde un PIN. Recorre todas las marcas siempre —sin cortar al
// encontrarla— para que el tiempo de respuesta no delate si el PIN existe.
async function resolverPin(pin) {
  const limpio = String(pin || '').trim();
  if (!/^\d{4,12}$/.test(limpio)) return null;
  const registro = await leerRegistro();
  let encontrado = null;
  if (registro.adminPin && igualSeguro(limpio, registro.adminPin)) encontrado = { token: registro.adminToken };
  (registro.marcas || []).forEach(m => {
    if (m.pin && igualSeguro(limpio, m.pin)) encontrado = { token: m.token };
  });
  return encontrado;
}
...
// PIN de 6 dígitos: un millón de combinaciones. Con el freno por intentos alcanza; con 4
// dígitos, no. Se evita que arranque en 0 para que nadie lo copie mal al dictarlo.
function pinAlAzar() {
  const n = require('crypto').randomInt(100000, 1000000);
  return String(n);
}
...
module.exports = {
  ...
  codigoAlAzar, leerClaves, resolverPin, registrarIntento, estaBloqueado, pinAlAzar,
  MAX_INTENTOS
};
